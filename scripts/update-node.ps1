...
Copy-Item -Path $sdk_dir/$version/x64/* -Destination lib/node/windows/x64 -Recurse *> $null

New-Item -Path lib/node/windows/x86 -ItemType Directory *> $null
Copy-Item -Path $sdk_dir/$version/ia32/* -Destination lib/node/windows/x86 -Recurse *> $null
